...
#define M_SEARCH_ANGLE_ACCURACY_MODE                  1420L
...
#define M_HAS_DONT_CARE_MASK                           2391   // Also defined in milmod.h
...
MIL_PAT_DLLFUNC void    MFTYPE MpatSaveNewA(               MIL_CONST_TEXTA_PTR        FileName,
                                                           MIL_ID             ContextPatId,
                                                           MIL_INT64          ControlFlag);

MIL_PAT_DLLFUNC MIL_ID  MFTYPE MpatRestoreNewA(            MIL_CONST_TEXTA_PTR        FileName,
                                                           MIL_ID             SysId,
                                                           MIL_INT64          ControlFlag,
                                                           MIL_ID*            ContextPatIdPtr);

MIL_PAT_DLLFUNC void    MFTYPE MpatStreamA(                MIL_TEXTA_PTR      MemPtrOrFileName,
                                                           MIL_ID             SysId,
                                                           MIL_INT64          Operation,
...
MIL_PAT_DLLFUNC void    MFTYPE MpatSaveNewW(               MIL_CONST_TEXTW_PTR FileName,
                                                           MIL_ID             ContextPatId,
                                                           MIL_INT64          ControlFlag);

MIL_PAT_DLLFUNC MIL_ID  MFTYPE MpatRestoreNewW(            MIL_CONST_TEXTW_PTR FileName,
                                                           MIL_ID             SysId,
                                                           MIL_INT64          ControlFlag,
                                                           MIL_ID*            ContextPatIdPtr);

MIL_PAT_DLLFUNC void    MFTYPE MpatStreamW(                MIL_TEXTW_PTR       MemPtrOrFileName,
                                                           MIL_ID             SysId,
                                                           MIL_INT64          Operation,
                                                           MIL_INT64          StreamType,
                                                           MIL_DOUBLE         Version,
...
inline void MFTYPE MpatStreamCpp(MilStreamParam MemPtrOrFileName,
                                   MIL_ID         SysId,
                                   MIL_INT64      Operation,
                                   MIL_INT64      StreamType,
                                   MIL_DOUBLE     Version,
                                   ...
                                   MIL_INT*       SizeByteVarPtr)
   {
   MpatStream(MemPtrOrFileName.m_Param, SysId, Operation, StreamType, Version, ControlFlag, ContextPatIdPtr, SizeByteVarPtr);
   }

#undef  MpatStream
#define MpatStream MpatStreamCpp
...
#if M_MIL_USE_UNICODE
inline void MpatSaveA(MIL_CONST_TEXTA_PTR        FileName,
                      MIL_ID             ContextPatId)
   {
...
inline void MpatSaveW(MIL_CONST_TEXTW_PTR FileName,
                      MIL_ID             ContextPatId)
   {
...
inline void MFTYPE MpatGetResultSafeType(MIL_ID ResultPatId, MIL_INT Index, MIL_INT64 ResultType, std::nullptr_t                );
#endif
...
#if M_MIL_SAFE_TYPE_M_NULL_PTR_TYPE_EXISTS
inline void MFTYPE MpatGetResultSafeType(MIL_ID ResultPatId, MIL_INT64 ResultType, std::nullptr_t                ) { MpatGetResultSafeType(ResultPatId, M_DEFAULT, ResultType, M_NULL); }
#endif
...
#if M_MIL_SAFE_TYPE_M_NULL_PTR_TYPE_EXISTS
inline MIL_INT MFTYPE MpatInquireSafeType(MIL_ID ContextOrResultPatId, MIL_INT Index, MIL_INT64 InquireType, std::nullptr_t            );
#endif
...
#if M_MIL_SAFE_TYPE_M_NULL_PTR_TYPE_EXISTS
inline MIL_INT MFTYPE MpatInquireSafeType(MIL_ID ContextOrResultPatId, MIL_INT64 InquireType, std::nullptr_t            ) { return MpatInquireSafeType(ContextOrResultPatId, M_DEFAULT, InquireType, M_NULL    ); }
#endif
...
inline void MFTYPE MpatGetResult(MIL_ID ResultId, MIL_INT Index, MIL_INT64  ResultType, std::vector<UserType> &ValuePtr)
   {
   ...
   if(M_GET_HLVLDATATYPE(ResultType) != 0)
      {
      SafeTypeError(MIL_TEXT("MpatGetResult"), MIL_TEXT("Combination value for the required data type is not supported with std::vector overload."));
      }
#endif
   ...
   MpatGetResult(ResultId, Index, M_STRIP_HLVLDATATYPE(ResultType) + M_NB_ELEMENTS + M_TYPE_MIL_INT, &InternalNumberOfElementsForStdVector);

   ValuePtr.resize(InternalNumberOfElementsForStdVector);

   if(InternalNumberOfElementsForStdVector > 0)
      {
      MpatGetResult(ResultId, Index, ResultType, &ValuePtr[0]);
      }
   }

// ----------------------------------------------------------
// Overloads for std::vector in MxxxStream.
inline void MFTYPE MpatStream(std::vector<MIL_UINT8>  &MemPtrOrFileName,
                               MIL_ID                 SysId,
                               MIL_INT64              Operation,
                               MIL_INT64              StreamType,
                               MIL_DOUBLE             Version,
...
#if M_MIL_USE_STRING
   /***************************************************************************/
   /* PATERN MATCHING MODULE                                                  */
   /***************************************************************************/
#if M_MIL_USE_UNICODE
#if M_MIL_UNICODE_API
   inline MIL_ID MFTYPE MpatRestoreW(const MIL_STRING& FileName, MIL_ID SysId, MIL_INT64 ControlFlag, MIL_ID* ContextPatIdPtr)
      {
      return MpatRestoreW(FileName.c_str(), SysId, ControlFlag, ContextPatIdPtr);
      }

   inline void MFTYPE MpatSaveW(const MIL_STRING& FileName, MIL_ID ContextPatId, MIL_INT64 ControlFlag)
      {
      return MpatSaveW(FileName.c_str(), ContextPatId, ControlFlag);
      }

#else
   inline MIL_ID MFTYPE MpatRestoreA(const MIL_STRING& FileName, MIL_ID SysId, MIL_INT64 ControlFlag, MIL_ID* ContextPatIdPtr)
      {
      return MpatRestoreA(FileName.c_str(), SysId, ControlFlag, ContextPatIdPtr);
      }

   inline void MFTYPE MpatSaveA(const MIL_STRING& FileName, MIL_ID ContextPatId, MIL_INT64 ControlFlag)
      {
      return MpatSaveA(FileName.c_str(), ContextPatId, ControlFlag);
      }


#endif /* M_MIL_UNICODE_API */
#else
   inline MIL_ID MFTYPE MpatRestore(const MIL_STRING& FileName, MIL_ID SysId, MIL_INT64 ControlFlag, MIL_ID* ContextPatIdPtr)
      {
      return MpatRestore(FileName.c_str(), SysId, ControlFlag, ContextPatIdPtr);
      }

   inline void MFTYPE MpatSave(const MIL_STRING& FileName, MIL_ID ContextPatId, MIL_INT64 ControlFlag)
      {
      return MpatSave(FileName.c_str(), ContextPatId, ControlFlag);
      }
...
#if M_MIL_USE_SAFE_TYPE
template <> inline bool MilIsCorrectObjectType<&MpatFree>(MIL_INT64 ObjectType)
   {
   return (ObjectType & ~M_USER_DEFINE_LOW_ATTRIBUTE) == M_PAT_OBJECT;
   }
#endif

typedef MIL_UNIQUE_ID<&MpatFree> MIL_UNIQUE_PAT_ID;

#if M_MIL_USE_MOVE_SEMANTICS

inline MIL_UNIQUE_PAT_ID MpatAlloc(MIL_ID SysId, MIL_INT64 ContextType, MIL_INT64 ControlFlag, MIL_UNIQUE_ID_TAG)
   {
   return MIL_UNIQUE_PAT_ID(MpatAlloc(SysId, ContextType, ControlFlag, M_NULL));
   }
inline MIL_UNIQUE_PAT_ID MpatAllocResult(MIL_ID SysId, MIL_INT64 ControlFlag, MIL_UNIQUE_ID_TAG)
   {
   return MIL_UNIQUE_PAT_ID(MpatAllocResult(SysId, ControlFlag, M_NULL));
   }
inline MIL_UNIQUE_PAT_ID MpatRestore(MIL_CONST_TEXT_PTR FileName, MIL_ID SysId, MIL_INT64 ControlFlag, MIL_UNIQUE_ID_TAG)
   {
   return MIL_UNIQUE_PAT_ID(MpatRestore(FileName, SysId, ControlFlag, M_NULL));
   }
#if M_MIL_USE_STRING
inline MIL_UNIQUE_PAT_ID MpatRestore(const MIL_STRING& FileName, MIL_ID SysId, MIL_INT64 ControlFlag, MIL_UNIQUE_ID_TAG)
   {
   return MIL_UNIQUE_PAT_ID(MpatRestore(FileName, SysId, ControlFlag, M_NULL));
   }
#endif // M_MIL_USE_STRING
template <MilFreeFuncType FreeFunc> inline void MpatFree(const MIL_UNIQUE_ID<FreeFunc>&) = delete;

inline void MpatStream(MilStreamParam     MemPtrOrFileName,
                       MIL_ID             SysId           ,
                       MIL_INT64          Operation       ,
                       MIL_INT64          StreamType      ,
                       MIL_DOUBLE         Version         ,
                       MIL_INT64          ControlFlag     ,
                       MIL_UNIQUE_PAT_ID* ContextPatIdPtr ,
                       MIL_INT*           SizeByteVarPtr  )
   {
   MxxxStreamForMilUniqueId(MemPtrOrFileName, SysId, Operation, StreamType, Version, ControlFlag, ContextPatIdPtr, SizeByteVarPtr, MpatStream);
   }
