...
IDSCaller::IDSCaller()
{
	m_pCamera = IDSCamera::GetInstance();
}
...
/*
	...
	其中参数1：IDSCaller为当前类的类名，参数2：Demo_msleep100为需要注册的函数名
*/
int IDSCaller::RegisterMethod()
{
	Plugin_Method_Add(IDSCaller, Demo_msleep100);
...
*/
int IDSCaller::Demo_msleep100(JupData & data)
{
	Sleep(100);
...
int IDSCaller::InitCamera(JupData & data)
{
	bool ret = m_pCamera->OpenCamera();
	return 0;
}

int IDSCaller::CloseCamera(JupData & data)
{
	m_pCamera->ExitCamera();
	return 0;
}

int IDSCaller::StartCaptureImage(JupData & data)
{
	return 0;
}

int IDSCaller::WaitSmuSignalCaptureImage(JupData & data)
{
	return 0;
}
