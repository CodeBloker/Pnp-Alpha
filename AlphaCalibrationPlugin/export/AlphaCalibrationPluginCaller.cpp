...
AlphaCalibrationPluginCaller::AlphaCalibrationPluginCaller()
{
}
...
/*
	...
	根据当前插件的实际需要选择添加内容
*/
int AlphaCalibrationPluginCaller::Initialize()
{
...
/*
	...
	示例：Plugin_Method_Add(AlphaCalibrationPluginCaller, Demo_msleep100);
	其中参数1：AlphaCalibrationPluginCaller为当前类的类名，参数2：Demo_msleep100为需要注册的函数名
*/
int AlphaCalibrationPluginCaller::RegisterMethod()
{
	Plugin_Method_Add(AlphaCalibrationPluginCaller, Demo_msleep100);
	Plugin_Method_Add(AlphaCalibrationPluginCaller, Demo_msleep500Error);
	Plugin_Method_Add(AlphaCalibrationPluginCaller, Demo_msleep1000);
	Plugin_Method_Add(AlphaCalibrationPluginCaller, Demo_select);
	return 0;
}
...
*/
int AlphaCalibrationPluginCaller::Demo_msleep100(JupData & data)
{
	Sleep(100);
...
int AlphaCalibrationPluginCaller::Demo_msleep500Error(JupData & data)
{
	Sleep(500);
...
int AlphaCalibrationPluginCaller::Demo_msleep1000(JupData & data)
{
	Sleep(1000);

	jCore->Logger("AlphaCalibrationPlugin.dll").LogInfo(__FUNCTION__, "sleep 1s...");

	return 0;
}

int AlphaCalibrationPluginCaller::Demo_select(JupData & data)
{
	static int i = 0;
	if (i < 2)
	{
